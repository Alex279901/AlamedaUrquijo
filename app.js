...
const SHEETS_API_URL = "https://script.google.com/macros/s/AKfycbzzVc8943cCIaF4SaihKmKAn7GiT-qnNskTbvUwxrecQBo4qPjPAZcL0KNOwnlTQGVJEg/exec";
...
const allowedBranches = new Set(["SG Alameda de Urquijo"]);
const branchAliases = {};

const brandConfig = {
  "Santa Gloria ESP": {
    cover: "assets/cover-sgesp.png",
    branches: ["SG Alameda de Urquijo"]
  }
};
...
const state = {
  user: null,
  activeModule: "sales",
  activeBrand: "Santa Gloria ESP",
  activeBranches: new Set(brandConfig["Santa Gloria ESP"].branches),
  chartMode: "sales",
...
const exactFormatter = new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR", minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
async function loadSheetData(manual = false) {
  ...
  try {
    ...
    const ventasRows = extractSheetRows(payload, sheetMap.ventas.tab, "ventas");
    const productosRows = extractSheetRows(payload, sheetMap.productos.tab, "productos");
    const horasRows = extractSheetRows(payload, sheetMap.horas.tab, "horas");
    if (!ventasRows.length && !productosRows.length && !horasRows.length) throw new Error("Sheets respondio, pero no encontre filas en C_Fecha, C_Producto o C_Hora.");
    sheetData.ventas = filterAllowedBranches(normalizeVentas(ventasRows));
    sheetData.productos = filterAllowedBranches(normalizeProductos(productosRows));
    sheetData.horas = filterAllowedBranches(normalizeHoras(horasRows));
...
function filterAllowedBranches(rows) {
  return rows.filter((row) => allowedBranches.has(row.sucursal));
}
...
function ensureModuleDateRange() {
  ...
  if (fechas.length < 7) {
    state.dateStart = fechas[0];
    state.dateEnd = fechas[fechas.length - 1];
    return;
  }
...
function renderBranchFilter() {
  ...
  const allSelected = hasMultipleBranches && branches.every((branch) => state.activeBranches.has(branch));
  els.branchFilter.innerHTML = `
    ${hasMultipleBranches ? `<button class="chip select-all ${allSelected ? "active" : ""}" type="button" data-toggle-branches>${allSelected ? "Deselect All" : "Select All"}</button>` : ""}
  ` + branches.map((branch) => `
